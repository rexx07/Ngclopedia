    ...
    internal static IServiceCollection AddNotifications(this IServiceCollection services, IConfiguration config)
    {
        ...
        else
        {
            ...
            if (backplaneSettings is null)
                throw new InvalidOperationException("Backplane enabled, " +
                                                    "but no backplane settings in config.");
            switch (backplaneSettings.Provider)
            {
                ...
                default:
                    throw new InvalidOperationException($"SignalR backplane Provider {backplaneSettings.Provider}" +
                                                        " is not supported.");
            }
